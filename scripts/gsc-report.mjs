...
//        export GSC_SA_KEY_FILE="C:/Users/waite/.secrets/<your-sa-key>.json"
//        # The site is baked into the npm script (sc-domain:skinoversteel.com);
//        # override with GSC_SITE_URL or a CLI arg if you ever need to.
//
// Run:  npm run gsc                 # uses the baked-in skinoversteel property
//       npm run gsc -- --days=28    # override the window
//       node scripts/gsc-report.mjs "sc-domain:skinoversteel.com" --days=90
  ...
  if (!SITE_URL) {
    fail(
      'No site URL. Pass one as the first arg or set GSC_SITE_URL.\n' +
        '  e.g. node scripts/gsc-report.mjs "sc-domain:skinoversteel.com"',
    )
  }
  if (!KEY_FILE) {
    fail(
